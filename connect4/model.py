...
def search(board, player_marker='X', max_depth=3, player_moves_first=True):  # -> DiGraph
    ...
    for move in board.moves_available(state=board.state):
        # Do move
        new_state = board.play_move(
            state=board.state, move=move, marker=turn_marker)
        # Check if game is finished after move
        is_finished = board.is_finished(new_state)

        # Add move node to graph
        n = n+1
        G.add_node(n, finished=is_finished,
                   player=non_turn_maker, state=new_state)
        G.add_edge(0, n, move=move)
        if is_finished:
            G.nodes[n]['result'] = {
                player_marker: 'won', opponent_marker: 'lost', 'draw': 'draw'}.get(is_finished)
            continue
    ...
    while depth < max_depth:
        ...
        for leave in leavelist:
            # Get parent state
            # G.nodes(data=True)[list(G.pred[leave])[0]]['state']
            parent_state = G.nodes(data=True)[leave]['state']
            for move in board.moves_available(parent_state):
                # Do move
                new_state = board.play_move(
                    parent_state, move=move, marker=turn_marker)
                # Check if game is finished after move
                is_finished = board.is_finished(new_state)
                # Add move node to graph
                n = n+1
...
class Board():
    ...
    def draw(state):
        ...
        for m in range(size):
            print('|'.join([value_to_marker[state[m][n]]
                            for n in range(size)]))
            if m != size-1:
                print('-'*((size*2)-1))
...
def minimax(Graph, first_move=True):
    ...
    for child in G.successors(0):
        v = _minimax(G, child, {True: minplayer,
                                False: maxplayer}.get(first_move))
        G.nodes[child].update(
            {'score': v, 'player': {True: 'min', False: 'max'}.get(first_move)})
